...
`;function RSAValidate(t){let e=RSAValidateError(t);e&&alert(e)}function RSAValidateError(t){let e=["id","rtl","title","nav","subtitle","AppContext","actions","body","header","headerContent","maxWidth","side","theme"];for(let r in t)if(-1===e.indexOf(r))return`
    ...
    `;if("function"!=typeof t.body)return`
      ...
      ${i}
    `;for(let s=0;s<t.items.length;s++){let a=t.items[s],{text:o,show:n=()=>!0,attrs:l={}}=a,p=["text","icon","attrs","show","onClick"];for(let h in a)if(-1===p.indexOf(h))return`
          react-super-app error => side.items[${s}].${h} is not a valid side item property.
          ${i}
    ...
    `;let e=["id","items","header","footer","cache","nested"];for(let r in t)if(-1===e.indexOf(r))return`
        react-super-app error => invalid nav property (${r}). 
        valid nav properties are ${e.split(" - ")}
  ...
  `;let i=RSAValidateNavItems(t.items);if(i)return i}function RSAValidateNavItems(t=[],e="nav"){let r=`
    ...
    ${RSANavItemInterface}
  `;for(let i=0;i<t.length;i++){let s=t[i],{id:a,text:o,show:n=()=>!0,render:l}=s,p=[],h=["id","items","icon","show","text","render"];for(let d in s)if(-1===h.indexOf(d))return`
      ...
      `;if(!a||"string"!=typeof a)return`
        ...
        ${r}
      `;if(-1!==p.indexOf(a))return`
        react-super-app error => ${e}.items[${i}].id is duplicate.
        ${r}
      `;if(p.push(s.id),!o||"string"!=typeof o)return`react-super-app error => ${e}.items[${i}].text should be an string`;let u=RSAValidateNavItems(s.items,e+`.items[${i}]`);if(u)return u}}